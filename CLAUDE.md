...
- **tailer.go** — two tailing strategies dispatched by `tailFile` / `tailStdin`:
  - Regular files: `emitLastN` (backward chunk scan via `lastNLines`) then `nxadm/tail` in follow+reopen mode starting at EOF.
...
Labels are user-owned strings and are written verbatim before each line — users include their own spacing/brackets (e.g. `"[db] "`). Auto-generated labels (basename/abspath) get a colon appended (e.g. `"app.log:"`). `abspath` resolves the path to absolute via `filepath.Abs` before appending the colon. ANSI color codes are pre-applied to labels in `main.go` (before passing to tailers) when stdout is a terminal and `--no-color` is not set.
